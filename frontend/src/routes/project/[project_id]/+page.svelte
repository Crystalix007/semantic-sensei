		...
		<div>
			...
			{:then data}
				...
				<div class="flex justify-between mb-1">
					...
					<a href="/project/{data.id}/label/create" class="block bg-teal-600 my-auto py-0.5 px-2 rounded-md border-blue-300 border text-white">New Label</a>
				</div>
				<table class="w-full pt-2 border border-zinc-300 rounded-md border-spacing-1 text-center">
					<thead>
						...
						{#each data.labels as label}
							<tr class="border border-zinc-300">
								<td class="border border-zinc-300">
									<a class="text-teal-600" href="/project/{data.id}/label/{label.id}">{label.id}</a>
								</td>
								<td class="border border-zinc-300">{label.label}</td>
								<td class="border border-zinc-300">{new Date(label.created_at).toLocaleString()}</td>
							</tr>
						{/each}
					</tbody>
				</table>
